...
mpoptli = mpoption('pf.alg', 'sicnm', 'verbose', 3,'out.all',0, ...
    'pf.nr.max_it', 5000, 'opf.violation', 5,...
    'fmincon.tol_x', 1e-1,...
    'fmincon.tol_f', 1e-1, ...
    'mips.sc.red_it',2, ...
    'mips.max_it',6.0,...
...
%% 2S4
mpoptli = mpoption('pf.alg', '2S4', 'verbose', 3,'out.all',0, 'pf.nr.max_it', 5000, ...
    'pf.tol', tol); 
pf_2s4=runpfa(scenario, mpoptli);
%% IWAMOTO
mpoptli = mpoption('pf.alg', 'IWAMOTO', 'verbose', 3,'out.all',0, 'pf.nr.max_it', 5000, ...
    'pf.tol', tol); 
pf_iwamoto=runpfa(scenario, mpoptli);
%% MANN
mpoptli = mpoption('pf.alg', 'MANN', 'verbose', 3,'out.all',0, 'pf.nr.max_it', 5000, ...
    'pf.tol', tol); 
pf_mann=runpfa(scenario, mpoptli);
%% ROMBERG
mpoptli = mpoption('pf.alg', 'ROMBERG', 'verbose', 3,'out.all',0, 'pf.nr.max_it', 5000, ...
    'pf.tol', tol); 
pf_romberg=runpfa(scenario, mpoptli);
%% NR
mpoptnr = mpoption('verbose', 3,'out.all',1,'opf.start',1, 'pf.nr.max_it', 5000, ...
    'pf.tol', tol); 
pf2=runpf(scenario, mpoptnr);
% 
% pf2=runpf(pf_rodas3d, mpoptnr);
