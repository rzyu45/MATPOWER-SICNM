function [MVAbase, bus, gen, branch, success, et] = ...
                runpfa(casedata, mpopt, fname, solvedcase)
    ...
    if dc                               %% DC formulation
        ...
        while (repeat)
            ...
            switch alg
                ...
                case 'SICNM'
                    [V, success, iterations, stats] = sicnm(Ybus, Sbus, V0, ref, pv, pq, mpopt);
                case 'CNM'
                    [V, success, iterations, stats] = cnm(Ybus, Sbus, V0, ref, pv, pq, mpopt);
                case '2S4'
                    [V, success, iterations, stats] = alg1_2s4(Ybus, Sbus, V0, ref, pv, pq, mpopt);
                case 'IWAMOTO'
                    [V, success, iterations, stats] = alg2_iwamoto(Ybus, Sbus, V0, ref, pv, pq, mpopt);
                case 'MANN'
                    [V, success, iterations, stats] = alg3_mann(Ybus, Sbus, V0, ref, pv, pq, mpopt);
                case 'ROMBERG'
                    [V, success, iterations, stats] = alg4_romberg(Ybus, Sbus, V0, ref, pv, pq, mpopt);
                case {'FDXB', 'FDBX'}
            ...
            %% update data matrices with solution
            switch alg
                case {'NR', 'NR-SP', 'NR-SC', 'NR-SH', 'NR-IP', 'NR-IC', 'NR-IH', 'FDXB', 'FDBX', 'GS', 'SICNM', 'CNM', '2S4', 'IWAMOTO', 'MANN', 'ROMBERG'}
...
if nargout == 1 || nargout == 2
    if any(contains(["cnm", "sicnm", 'IWAMOTO', '2S4', 'ROMBERG', 'MANN'], mpopt.pf.alg))
        results.stats=stats;
    end
    MVAbase = results;
    bus = success;
